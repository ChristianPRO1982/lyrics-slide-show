...
Adding a secret to a `private` group does not change existing membership rows.

It only adds a secret-based selection path for people who are not members or not authenticated.
