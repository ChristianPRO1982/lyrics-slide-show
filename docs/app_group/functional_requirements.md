...
For PostgreSQL, the functional tables of `app_group` are now shared tables in the `common` schema:

- `common.g_groups`,
- `common.g_group_user`,
- `common.g_group_user_ask_to_join`.

These tables are common to `Lyrics Slide Show` and `Animation-messe`.
...
CREATE INDEX g_groups_status_idx ON common.g_groups (status);
...
CREATE INDEX g_group_user_is_group_admin_idx ON common.g_group_user (is_group_admin);

CREATE INDEX g_group_user_am_access_idx ON common.g_group_user (am_access);
...
- `g_group_user.am_access` is reserved for `Animation-messe`; `Lyrics Slide Show` must ignore it, and membership in LSS is defined only by the existence of the `(group_id, member_id)` row,
