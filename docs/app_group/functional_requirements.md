...
- the secret must be generated by the system only,
- no manual secret entry is required in the product workflow,
...
- secret-based access granted to a non-member is temporary and must expire automatically after a configurable duration,
- that maximum duration must be configurable through `.env`,
- after that duration expires, the secret must be entered again to restore access,
- rotating or removing the secret does not require immediate global revocation of already-open secret-based sessions if those sessions are still within their allowed lifetime,
