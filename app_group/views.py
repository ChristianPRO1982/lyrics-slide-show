...
def _build_groups_page_context(
    request: HttpRequest, create_form: GroupCreateForm | None = None
) -> dict[str, object]:
    ...
    if member_id:
        memberships_by_group_id = {
            ...
            for membership in GroupMembership.objects.filter(
                group_id__in=group_ids, member_id=member_id
            )
        }
    ...
    admin_memberships = list(
        GroupMembership.objects.filter(
            group_id__in=group_ids, is_group_admin=True
        ).order_by("group_id", "member_id")
    )
    admin_users = fetch_directory_users(
        [membership.member_id for membership in admin_memberships]
    )
    ...
    for group in groups:
        ...
        can_select = user_can_select_group(
            request.user, group, membership, active_secret
        )
        is_member = membership is not None
        is_group_admin = bool(membership and membership.is_group_admin)
        can_view_admin_usernames = bool(
            is_member or can_manage_groups_globally(request.user)
        )
        group_cards.append(
            {
                "group": group,
                ...
                "has_secret": bool(
                    group.secret_ciphertext and group.status == GroupStatus.PRIVATE
                ),
                "membership_marker": "👩🏾‍🔧"
                if is_group_admin
                else "👥"
                ...
                if group.group_id in pending_request_group_ids
                else "",
                "membership_label": (
                    _("Responsable")
                    if is_group_admin
                ...
                "can_request_join": bool(
                    member_id
                    and not is_member
                    and group.group_id not in pending_request_group_ids
                    and group.status == GroupStatus.PRIVATE
                ),
                "can_cancel_request": bool(
                    member_id and group.group_id in pending_request_group_ids
                ),
                "can_leave": bool(is_member),
                "needs_secret_prompt": bool(
                    not can_select and business_status == "private_with_secret"
                ),
                "is_selected": bool(
                    selected_group and selected_group.group_id == group.group_id
                ),
            }
        )
...
def _handle_group_link_selection(request: HttpRequest) -> HttpResponse | None:
    ...
    if selected_group_id:
        ...
        group = get_object_or_404(
            Group, secret_ciphertext=provided_secret, status=GroupStatus.PRIVATE
        )
    member_id = get_member_id_from_user(request.user)
    membership = None
    if member_id:
    ...
    if membership is not None:
        ...
        and provided_secret
        and provided_secret == group.secret_ciphertext
    ):
        select_group(request.session, group, access_secret=provided_secret)
        messages.success(
            request, _("Le groupe sélectionné a été mis à jour grâce au secret.")
        )
    else:
...
def groups(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if action == "create_group":
            if not member_id:
                messages.error(
                    request, _("Vous devez être connecté pour créer un groupe.")
                )
                return redirect("groups")
                ...
                if _duplicate_name_exists(create_form.cleaned_data["name"]):
                    ...
                    try:
                        ...
                        create_form.add_error(
                            "name", _("Ce nom de groupe existe déjà.")
                        )
                    else:
                        ...
                        select_group(request.session, group)
                        messages.success(
                            request, _("Le groupe a été créé et sélectionné.")
                        )
                        return redirect("modify_group", group_id=group.group_id)
            return render(
                request,
                "group/groups.html",
                _build_groups_page_context(request, create_form=create_form),
            )
        ...
        if action == "select_group":
            ...
            if group.status == GroupStatus.OPEN or membership is not None:
                ...
                group.secret_ciphertext and submitted_secret == group.secret_ciphertext
            ):
                select_group(request.session, group, access_secret=submitted_secret)
                messages.success(
                    request,
                    _("Le groupe sélectionné a été mis à jour grâce au secret."),
                )
            else:
                messages.error(
                    request,
                    _("Ce groupe n'est pas accessible avec les informations fournies."),
                )
            return redirect("groups")
        ...
        if action == "request_join":
            if not member_id:
                messages.error(
                    request,
                    _("Vous devez être connecté pour demander à rejoindre un groupe."),
                )
            elif group.status != GroupStatus.PRIVATE:
                messages.error(
                    request,
                    _("Les demandes d'adhésion sont réservées aux groupes fermés."),
                )
        ...
        if action == "cancel_request":
            if not member_id:
                messages.error(
                    request, _("Vous devez être connecté pour annuler une demande.")
                )
            else:
                GroupJoinRequest.objects.filter(
                    group_id=group.group_id, member_id=member_id
        ...
        if action == "leave_group":
            if not member_id or membership is None:
                ...
                try:
                    ...
                    selected_group, _selected_via_secret = get_selected_group_state(
                        request
                    )
...
def modify_group(request: HttpRequest, group_id: int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if action == "save_group_settings":
            ...
            if settings_form.is_valid():
                if _duplicate_name_exists(
                    settings_form.cleaned_data["name"], exclude_group_id=group.group_id
                ):
                    add_duplicate_name_message(request)
                    ...
                    try:
                        ...
                        messages.success(
                            request, _("Les paramètres du groupe ont été enregistrés.")
                        )
                        return redirect("modify_group", group_id=group.group_id)
            ...
            if (
                request.session.get(SELECTED_GROUP_ID_SESSION_KEY) == group.group_id
                and membership is None
            ):
                clear_selected_group(request.session)
            messages.success(
                request, _("Le secret du groupe a été généré ou remplacé.")
            )
            return redirect("modify_group", group_id=group.group_id)
            ...
            if (
                selected_group
                and selected_group.group_id == group.group_id
                and selected_via_secret
            ):
                clear_selected_group(request.session)
            ...
            target_member_id = request.POST.get("member_id", "")
            GroupJoinRequest.objects.filter(
                group_id=group.group_id, member_id=target_member_id
            ).delete()
            ...
            expected_word = get_delete_confirmation_word(
    ...
    directory_users = fetch_directory_users(
        ...
        + [join_request.member_id for join_request in join_requests]
    )
    ...
    context = {
        ...
        "delete_confirmation_word": get_delete_confirmation_word(
            getattr(request, "LANGUAGE_CODE", None)
        ),
        ...
        "can_select_current_group": user_can_select_group(
            request.user, group, membership, current_session_secret
        ),
    }
